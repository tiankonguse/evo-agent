...
- **Custom Subagents (`.evo-agent/agents/`)**: Define named specialists in Markdown + YAML frontmatter — drop a file at `.evo-agent/agents/<name>.md` and the model can invoke it via `task({subagent_type: "<name>", prompt: "..."})`. Unlike the generic subagent, custom agents **fully replace** the parent's system prompt with their own body (only a minimal environment envelope is appended), and may override the model and `max_turns` via frontmatter. Available agent types are auto-injected into the main agent's system prompt as an `agents_catalog` section so the model knows which `subagent_type` values are valid. Inspired by Claude Code's `.claude/agents/`. Full guide: [`docs/CUSTOM_AGENTS.md`](docs/CUSTOM_AGENTS.md)
- **Fork Subagents**: `task({fork: true, prompt: <directive>})` spawns a child that inherits the parent's **full** system prompt AND complete message history, then receives a directive as a final user message — useful for survey questions ("audit ship readiness") or implementation work where the intermediate tool output isn't worth keeping in your context. Mutually exclusive with `subagent_type`. Pre-flight `FilterIncompleteToolCalls` strips orphan tool_use blocks from the parent's mid-turn assistant message so the API doesn't 400. Recursion is blocked by stripping `task` from the fork's tool list plus a `<fork-boilerplate>` directive that tells the model not to spawn sub-agents. Cap of 60 LLM round-trips (vs 30 generic). Inspired by Claude Code's `forkSubagent`
- **TUI Subagent Visual Separation**: Every event emitted by a subagent / fork is tagged with `Event.AgentName`; both the plain TerminalSink and the Bubble Tea sink render those events with a colored gutter bar (`┃ `) and a `[<agent-name>]` header on the first line, so the user can attribute output at a glance. Colors are FNV-hash → bold-bright palette: same name → same color across the session; different agents pick distinct colors
- **`/agents` Command**: Pure-client slash command for inspecting custom subagents — `/agents` (list every loaded agent with name + description + model + max_turns), `/agents show <name>` (frontmatter + full system prompt body), `/agents reload` (re-scan `.evo-agent/agents/` after editing files, no restart needed). Never drives an LLM turn. Auto-completes in the TUI's `/` dropdown alongside `/tools` / `/goal` / etc.
...
└── internal/
    ...
    │   ├── subagent.go        # RunSubagent + shared runSubagentLoop helper (used by all 3 subagent flavors)
    │   ├── custom_subagent.go # RunCustomSubagent: named agent from .evo-agent/agents/, replaces parent prompt
    │   ├── fork.go            # RunForkSubagent: inherits parent's full prompt + history, with <fork-boilerplate> directive
    │   ├── filter.go          # FilterThinking + FilterIncompleteToolCalls (strips orphan tool_use for fork)
    │   ├── agentscmd.go       # /agents client-side slash command: list / show / reload — never drives an LLM turn
    ...
    ├── agents/
    │   └── registry.go        # AgentDefinition, Init, Get, List, Names, Catalog — loads .evo-agent/agents/<name>.md
    ├── config/
...
| `task`        | Spawn a subagent with fresh context to complete a subtask; shares filesystem but not history; returns a text summary; max 30 turns. Optional `subagent_type: "<name>"` invokes a custom agent from `.evo-agent/agents/` (replaces parent prompt). Optional `fork: true` inherits the parent's full prompt + message history and acts on the prompt as a directive (max 60 turns; mutually exclusive with `subagent_type`) |
...
Custom subagents are named specialists you define in Markdown — drop a file at `.evo-agent/agents/<name>.md` and the model can invoke it via `task({subagent_type: "<name>", prompt: "..."})`.
...
---
name: code-reviewer
description: Senior code-reviewer that finds bugs, security issues, and style violations
model: inherit
max_turns: 20
---
...
|---|---|---|---|
| `name` | No | filename minus `.md` | Identifier used as `subagent_type` |
| `description` | Yes | `"No description"` | One-line summary advertised to the main agent |
| `model` | No | parent's model | Anthropic model id; `inherit` or empty = parent's |
| `max_turns` | No | 30 | LLM round-trip cap |

Three subagent flavors compared:

| | Generic (`subagent_type` omitted) | Custom (`subagent_type: "<name>"`) | Fork (`fork: true`) |
|---|---|---|---|
| System prompt | Parent + addendum | **Agent's own body** + minimal env | **Parent's full prompt** |
| Message history | Just the user prompt | Just the user prompt | **Parent's full history** + directive |
| Model | Parent's | frontmatter `model` overrides | Parent's |
| Max turns | 30 | frontmatter `max_turns` overrides | 60 |
| Tools | `ToolsExcept("task")` | Same | Same (recursion guard) |
| Use case | Generic delegation | Specialist (review / explore / test) | Survey / continuation work |

`/agents` slash commands (pure client-side, never drive an LLM):

```
/agents                   # list every loaded agent (name, description, model, max_turns)
/agents show code-reviewer # show full frontmatter + system prompt body
/agents reload            # re-scan .evo-agent/agents/ after editing files
```

The TUI renders subagent / fork output with a colored gutter (`┃ [<name>] ...`) so you can tell at a glance which agent emitted which line. Same agent → same color across the session; different agents pick distinct colors from a 6-entry bold-bright palette.

Full guide: [`docs/CUSTOM_AGENTS.md`](docs/CUSTOM_AGENTS.md).
...
| Version | Description |
|---------|-------------|
| **v0.20.0** | Add custom subagents + fork + TUI visual separation + `/agents` command. **(1) Custom subagents**: drop a Markdown file at `.evo-agent/agents/<name>.md` with YAML frontmatter (`name`, `description`, optional `model`, optional `max_turns`) and the model can invoke it via `task({subagent_type: "<name>", prompt: "..."})`. New `internal/agents/` leaf package owns `Init` / `Get` / `List` / `Names` / `Catalog`; `tools/task.go` adds the `subagent_type` field + `RegisterCustomSubagentRunner` callback; `agent/custom_subagent.go` adds `RunCustomSubagent` which **completely replaces** the parent's system prompt with the agent's body + a minimal environment envelope (working dir / git / platform / shell / date / model) — Agent.md / skills catalog / memories / plans / team / goal are deliberately NOT inherited so each agent stays focused. `prompt/builder.go` gains an `AgentsProvider` interface and an `agents_catalog` dynamic section so the model sees which subagent_types exist. **(2) Fork subagents**: new `task({fork: true, prompt: <directive>})` path inherits the parent's full system prompt AND message history and acts on the prompt as a directive — useful for survey questions ("audit ship readiness") or implementation work where intermediate tool noise isn't worth keeping. New `internal/agent/fork.go` with `RunForkSubagent`, `<fork-boilerplate>` directive prefix telling the model not to spawn sub-agents and to output `Scope:` / `Result:` / etc. New `agent/filter.go::FilterIncompleteToolCalls` strips orphan tool_use blocks from the parent's mid-turn assistant message so the API doesn't 400 (mid-turn fork is the common case). 60-turn cap (vs 30 generic). Mutually exclusive with `subagent_type`. **(3) TUI subagent visual separation**: new `Event.AgentName` field + scoped `Print*As(agentName, ...)` helpers (`PrintTextAs`, `PrintToolCallAs`, `PrintToolResultAs`, `PrintCommandAs`, `PrintErrorAs`, `PrintSystemAs`, `PrintTokensAs`); `runSubagentLoop` switches from string-prefixing (`[name] ...`) to setting `Event.AgentName`; both `TerminalSink` (plain mode) and the Bubble Tea sink (`tui/model.go::processEvent` + new `tui/subagent.go::indentSubagent`) prepend a colored gutter `┃ ` + `[<name>]` header to every emitted block. Colors come from FNV-hash → bold-bright palette (lipgloss hex `#ffd000`/`#00ff95`/`#ff7eff`/`#ff5555`/`#00e5ff`/`#ff9b3f` for the TUI; ANSI `[1;9X]` bold-bright for plain) so the same agent always gets the same color and different agents pick distinct colors. `tui/blocks.go::Block.AgentName` carries the tag through the tool-call pending list so the printed result inherits the gutter. Bonus dark-mode legibility fix: `tui/styles.go::systemStyle` foreground brightened from `#484f58` (illegible gray on dark themes) → `#8b949e` (matches `toolArgsStyle`); affects `/agents` / `/tools` / `/team` list output, subagent turn markers, token-usage debug lines, errors. **(4) `/agents` command**: new `agent/agentscmd.go` (`ParseAgentsCmd` + `handleAgentsCmd`) intercepted in `repl.go` before the skills registry; `/agents` and `/agents list` show the roster (`name [model=… max_turns=…] description`), `/agents show <name>` prints the full frontmatter + system prompt body (truncated at 2000 chars with a path footer for the rest), `/agents reload` re-scans `.evo-agent/agents/` so users can edit files without restarting. The TUI's `/` autocomplete dropdown injects `agents` alongside `tools` (`tui/model.go::NewModel`). Eleven new tests across `internal/agents/` (3) / `internal/agent/` (4 FilterIncompleteToolCalls + 7 ParseAgentsCmd) / `internal/ui/` (4 IndentSubagent / SubagentColor). New docs `docs/CUSTOM_AGENTS.md` (full guide) + REFERENCE.md updates. Inspired by Claude Code's `.claude/agents/` directory loader (`refs/loadAgentsDir.ts`), `forkSubagent.ts`, and the `/agents` slash command |
